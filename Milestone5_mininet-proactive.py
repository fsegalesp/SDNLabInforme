...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch2_features_handler(self, ev):
        ...
        match = parser.OFPMatch(in_port=1, vlan_vid = (0x1000 | 200))
        actions = [parser.OFPActionPopVlan(), parser.OFPActionOutput(3)]
        self.add_flow(datapath, 950, match=match, actions=actions)

        match = parser.OFPMatch(in_port=3)
        actions = [parser.OFPActionPushVlan(), parser.OFPActionSetField(vlan_vid=(0x1000 | 200)), parser.OFPActionOutput(1)]
        self.add_flow(datapath, 950, match=match, actions=actions)
